...
import com.example.meettingflat.Utils.PixelUtils;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.device_gridPasswordView, defStyleAttr, 0);

        mTextColor = ta.getColorStateList(R.styleable.device_gridPasswordView_device_gpvTextColor);
        if (mTextColor == null)
            mTextColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.device_gridPasswordView_device_gpvTextSize, -1);
        if (textSize != -1) {
            this.mTextSize = PixelUtils.px2sp(context, textSize);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.device_gridPasswordView_device_gpvLineWidth, PixelUtils.dip2px(getContext(), 1));
        mLineColor = ta.getColor(R.styleable.device_gridPasswordView_device_gpvLineColor, DEFAULT_LINECOLOR);
        mGridColor = ta.getColor(R.styleable.device_gridPasswordView_device_gpvGridColor, DEFAULT_GRIDCOLOR);
        mLineDrawable = ta.getDrawable(R.styleable.device_gridPasswordView_device_gpvLineColor);
        if (mLineDrawable == null)
            mLineDrawable = new ColorDrawable(mLineColor);
        mOuterLineDrawable = generateBackgroundDrawable();

        mPasswordLength = ta.getInt(R.styleable.device_gridPasswordView_device_gpvPasswordLength, DEFAULT_PASSWORDLENGTH);
        mPasswordTransformation = ta.getString(R.styleable.device_gridPasswordView_device_gpvPasswordTransformation);
        if (TextUtils.isEmpty(mPasswordTransformation))
            mPasswordTransformation = DEFAULT_TRANSFORMATION;


        mPasswordType = ta.getInt(R.styleable.device_gridPasswordView_device_gpvPasswordType, 0);
    ...
    private void inflaterViews(Context context) {
        ...
        inflater.inflate(R.layout.device_merge_grid_password_view, this);
        ...
        while (index < mPasswordLength) {
            View dividerView = inflater.inflate(R.layout.device_view_pwd_default_divider, null);
            ...
            TextView textView = (TextView) inflater.inflate(R.layout.device_tv_pwd_view_text, null);
...
//        Class<ImeDelBugFixedEditText> cls=ImeDelBugFixedEditText.class;
//        try {
...
//        }
    ...
    public ImeDelBugFixedEditText getInput(){
        return mInputView;
    }
